...
namespace Ut {		
	...
	* 
	* Description: 
	*   Object for handling Observable measurements
	* 
	*/
	struct Observable {
		...
		* 
		* Throws : std::runtime_error if the val array doesn't correspond to the right Observable (size mismatch)
		*
		* Description: 
		*   Stores the measurements and adds +1 to the number of measurements
		*/
		void add(std::valarray<double> const& val) { 
			if(obs_.size() == 0) 
		...
		*	We then add the entry to the output file
		*/
		void reduce(json_spirit::mValue& jEntry) {
			if(!counter_)
	...
	*/
	struct Simulation {
		Simulation(std::string inputFolder,std::string outputFolder,std::string name) : name_(name), outputFolder_(outputFolder) {
			...
			jParams_["SEED"] = jParams_.at("SEED").get_int() + mpi::rank();//To have different simulations on all the processors
		};
