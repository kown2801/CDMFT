...
def load_matsubara_measurements_from_simulations(dataFolder,measurement_name,converged_from):
    i=0
    G = []
    try:
        while(True):
            G_total = np.loadtxt("./" + dataFolder + measurement_name + str(i) + ".dat")
            G.append(G_total[:,1:])
            omegas = G_total[:,0]
            #We have G(iwn)(matrix)
            i+=1
    except:
        pass
    #Now get the complex
    ...
    G = G[:,:,::2] + 1j*G[:,:,1::2]
    G = get_mean(G,converged_from)
    G = np.insert(G,0,omegas,axis=1)
    return G #Returns G(iwn)(site)

def get_mean(G,converged_from):
    return np.mean(G[converged_from:],axis=0)
