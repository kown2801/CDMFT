...
def order_and_remove_occupation(files_dir):
    ...
    try:
        ...
        pass
    return 0
def handle_occupation(files_dir,additional_arguments):
    order_and_remove_occupation(files_dir)
...
def delete_last(files_dir,additional_arguments):
...
def prepare_copy(folder_name,additional_arguments):
...
def compute_order_parameter(folder_name,additional_arguments): 
    filename = "green"
...
def store(folder_name,additional_arguments):
	arguments = json.loads(additional_arguments)
	if "converged_from" in arguments and "sorting_parameters_values" in arguments:
		dbh.move_and_add_folder(folder_name,arguments["converged_from"],arguments["sorting_parameters_values"])
	else:
		raise Exception("converged_from and sorting_parameters_values missing from the additionnal arguments for storing")


actions_list = ["occupations","delete_last","prepare_copy","order_parameter","store"]
actions_functions = [handle_occupation,delete_last,prepare_copy,compute_order_parameter,store]
...
parser.add_argument("--args",dest="additionnnal_arguments",help="some additionnal arguments in the form of a serialized json string",required=False)
...
actions_functions[index_in_list](path_from_main_to_files,args.additionnnal_arguments)
